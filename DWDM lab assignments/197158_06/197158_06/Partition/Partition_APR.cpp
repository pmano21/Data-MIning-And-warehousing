...
{ //gc=given Candidate set
	...
	for (int i = 0; i < gc.size(); i++)
	{
		...
		advance(ti, i);
		s = ti->first;
		set<int>::iterator itr = s.begin(), tsi; //temp set iterator
		int sup = 0;
		for (int j = low; j < high; j++)
		{
			int v1, v2; //2 variables
...
int main()
{
	...
	//fin.open("bookex.txt");N=3;
	//fin.open("APR_Randinput.txt");N=10;
	fin.open("create_input_file.txt");
	N = 5;
	...
	int ps = siz / N; //partition size;

	//fin.open("bookex.txt");
	//fin.open("APR_Randinput.txt");
	fin.open("create_input_file.txt");
	...
	int ii = 0; //ii represents which row of transaction is being dealt with
